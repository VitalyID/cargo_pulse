...
describe('SvgSprite', () => {
  ...
  beforeEach(async () => {
    ...
    componentCdRef = debugElement.injector.get(ChangeDetectorRef);
  });
  ...
  it('should set default href attribute', () => {
    const useElement: DebugElement = debugElement.query(By.css('use'));
    expect(useElement.nativeElement.getAttribute('href')).toBe(
      '/assets/icons/symbol-defs.svg#icon-hash',
    );
  });
  ...
  it('should set user href attribute', () => {
    ...
    const useElement: DebugElement = debugElement.query(By.css('use'));
    expect(useElement.nativeElement.getAttribute('href')).toBe(
      '/assets/icons/symbol-defs.svg#icon-grid',
    );
  });
});
